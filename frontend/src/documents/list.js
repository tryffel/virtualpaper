...
const DocumentPagination = (props) => (
...
const DocumentGrid = () => {
  ...
  return ids ? (
    <ThemeProvider theme={theme}>
      <Grid
        style={{ background: theme.palette.background.default, margin: "1em" }}
      >
        {ids.map((id) => (
...
export const LastUpdatedDocumentsList = (props) => {
  const [page, setPage] = useState(1);
  const [perPage, setPerPage] = useState(10);
  const [sort, setSort] = useState({ field: "updated_at", order: "DESC" });
  const { data, total, loading, error } = useQueryWithStore({
