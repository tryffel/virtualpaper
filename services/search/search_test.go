...
func Test_parseFilter(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			args: args{`key:value OR another:"multi word value" AND more:12 third:add`},
			want: &searchQuery{
				RawQuery:       `key:value OR another:"multi word value" AND more:12 third:add`,
				MetadataQuery:  []string{`metadata="key:value"`, "OR", `metadata="another:multi_word_value"`, "AND", `metadata="more:12"`, "AND", `metadata="third:add"`},
				MetadataString: `metadata="key:value" OR metadata="another:multi_word_value" AND metadata="more:12" AND metadata="third:add"`,
				Query:          "",
			},
			wantErr: false,
		},
